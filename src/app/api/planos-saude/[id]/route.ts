...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [healthPlan] = await db.select().from(healthPlansTable).where(eq(healthPlansTable.id, id)).limit(1)
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [existingHealthPlan] = await db.select().from(healthPlansTable).where(eq(healthPlansTable.id, id)).limit(1)
