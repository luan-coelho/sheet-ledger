...
interface ColumnActionsProps {
  ...
  onDelete: (healthPlan: HealthPlan) => void
}

function ColumnActions({ healthPlan, onEdit, onDelete }: ColumnActionsProps) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={() => onEdit(healthPlan)}>
          <Pencil className="mr-2 h-4 w-4" />
          Editar
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => onDelete(healthPlan)} className="text-destructive focus:text-destructive">
  ...
  return [
    {
      ...
      cell: ({ row }) => {
        ...
        return <span className="font-medium">{healthPlan.name}</span>
      },
    ...
    /* {
      ...
      cell: ({ row }) => {
        const healthPlan = row.original
        return <ColumnActions healthPlan={healthPlan} onEdit={actions.onEdit} onDelete={actions.onDelete} />
      },
    },
  ]
}
