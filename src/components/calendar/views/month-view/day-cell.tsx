...
export function DayCell({ cell, events, eventPositions }: IProps) {
  ...
  const cellContent = useMemo(
    () => (
      <motion.div
        className={cn('flex h-full flex-col gap-1 border-t border-l lg:min-h-[10rem]', isSunday(date) && 'border-l-0')}
        ...
        <DroppableArea date={date} className="h-full w-full py-2">
          ...
          {showMobileMore && (
            <div className="mx-2 flex items-end justify-end">
              <span className="text-accent-foreground text-[0.6rem] font-semibold">+{showMoreCount}</span>
            </div>
          )}
    ...
    [date, day, currentMonth, cellEvents, showMobileMore, showDesktopMore, showMoreCount, renderEventAtPosition],
  )
