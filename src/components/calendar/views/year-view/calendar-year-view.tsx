      ...
      <motion.div
        ...
        {MONTHS.map((month, monthIndex) => {
          ...
          return (
            <motion.div
              key={month}
              className="border-border flex flex-col overflow-hidden rounded-lg border shadow-sm"
              ...
              {/* Month header */}
              <div
                className="hover:bg-primary/20 cursor-pointer px-3 py-2 text-center text-sm font-semibold transition-colors sm:text-base"
              ...
              <div className="text-muted-foreground grid grid-cols-7 py-2 text-center text-xs font-medium">
                {WEEKDAYS.map(day => (
                    ...
                    <div
                      ...
                      className={cn(
                        ...
                        hasEvents && isCurrentMonth
                          ? 'hover:bg-accent/20 cursor-pointer hover:rounded-md'
                          : 'cursor-default',
                      )}>
                      {isCurrentMonth && hasEvents ? (
                          ...
                          <div className="flex h-full w-full flex-col items-center justify-start gap-0.5">
                            ...
                            <div className="flex items-center justify-center gap-0.5">
                              {dayEvents.length <= 2 ? (
                                dayEvents
                                  .slice(0, 2)
                                  .map(event => <EventBullet key={event.id} color={event.color} className="size-1.5" />)
                              ) : (
                        ...
                        <div className="flex h-full w-full flex-col items-center justify-start">
                          <span className={cn('flex size-5 items-center justify-center font-medium')}>{cell.day}</span>
                        </div>
                      )}
                    </div>
                  )
